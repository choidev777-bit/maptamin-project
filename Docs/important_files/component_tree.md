...
15. [Shared UI Primitives](#15-shared-ui-primitives)
16. [Custom Hooks](#16-custom-hooks)
17. [Component Directory Index](#17-component-directory-index)
...
│       │   │   └── 맵타민 SVG 로고
│       │   │
│       │   ├── DesktopNav [Client] (md:block)
│       │   │   ├── 네비게이션 링크 (대시보드, 진단 기록, 내 순위 검색, 설정)
│       │   │   ├── NavDropdown [Client]
│       │   │   │   └── "내 순위 검색" 드롭다운 (네이버/구글)
│       │   │   ├── WalletLabel [Client]
│       │   │   │   └── 잔여 티켓 수 표시 (naver/google)
│       │   │   │   └── isSubscribed → 티켓 잔량 배지 / 미구독 → "구독 필요" 버튼
│       │   │   └── 사용자 이름 표시
│       │   │
...
| `dashboard/` | 24 | 대시보드 핵심 | `DashboardPlatformCard`, `SubscriptionContent`, `SearchHistorySection`, `PlanCard`, `CheckoutContent` |
| `history/` | 3 | 진단 기록 | `HistoryPageContent`, `RankTrendChart`, `HistoryTable` |
| `landing/` | 12 | 랜딩 페이지 | `Navigation`, `HeroSection`, `PricingSection`, `Footer` |
| `layout/` | 5 | 레이아웃 공통 | `DesktopNav` (진단 기록 링크 포함), `MobileNav` (진단 기록 링크 포함), `OnboardingGuard`, `WalletLabel`, `NavDropdown` |
...
graph TD
    ...
    DL --> OG --> DP & SS & CO & OB & NS & NR & GS & GR & ST & HI
    DL --> DN & MN
    DN --> WL & ND
    DL --> ML
