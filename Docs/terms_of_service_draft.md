...
4. 이용자는 이용자에게 부여된 아이디와 비밀번호를 직접 관리해야 합니다. 이용자의 관리 소홀로
발생한 문제는 회사가 책임을 부담하지 않습니다.
...
5. 정기 결제 상품(구독)의 경우, 이용자가 별도의 해지 신청을 하지 않는 한 매월(또는 매년) 동일한 날짜에 자동으로 결제가 이루어집니다. 카드 한도 초과, 잔액 부족 등으로 결제가 실패할 경우 서비스 이용이 자동으로 중지될 수 있습니다.
